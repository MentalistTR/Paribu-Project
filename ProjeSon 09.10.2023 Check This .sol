...
pragma solidity ^0.8.7;
 ...
 contract Project1 {
   ...
   function setHouseShop (
    ...
    function withdraw(address payable _address,uint _amount,address ownerEth) payable external {
        require(msg.sender == owner,"You are not owner");
        require(!locked, "Reentrancy call detected!");
        locked = true;   
        payable(_address).transfer(_amount);
        locked = false;   
        balances[ownerEth] -= _amount;
    }
    ...
    function getRent (
       ...
       external payable Owner checkSender(_leaser) {
         ...
         require(balances[msg.sender] >= _leaserAmount,"insufficient funds");
         ...
         _owner.transfer(_leaserAmount * monthNumber);
         balances[_leaser] -= _leaserAmount * monthNumber ;
